...
const EMPTY_MEMBER = {
  name: '', role: '', department: 'management', branch: '', image: '', sort_order: 0
};

export default function TeamFormModal({ open, onClose, onSave, member }) {
  const {
    state: form,
    setState: setForm,
    undo,
    ...
    canRedo,
    reset
  } = useUndoRedo(member || EMPTY_MEMBER);

  React.useEffect(() => {
    reset(member || EMPTY_MEMBER);
  }, [member, open, reset]);
  ...
  const handleChange = (e) => {
    ...
    setForm({ ...form, [name]: name === 'sort_order' ? Number(value) : value }, false);
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div>
            ...
            <input name="name" value={form.name} onChange={handleChange} required className={inputCls} placeholder="e.g. Mrs. Sangeetha" />
          </div>

          <div>
            <label className={labelCls}>Role / Designation *</label>
            <input name="role" value={form.role} onChange={handleChange} required className={inputCls} placeholder="e.g. Dental Assistant" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input name="branch" value={form.branch} onChange={handleChange} className={inputCls} placeholder="e.g. Jerush Head Clinic – Thuckalay" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-[1.6fr_1fr] gap-6">
          ...
          <div>
            <label className={labelCls}>Photo Preview & Order</label>
            <div className="flex gap-4 items-center">
              ...
              <div className="flex-1">
                <input name="sort_order" type="number" min="0" value={form.sort_order} onChange={handleChange} className={inputCls} placeholder="0" />
                <p className="text-[9px] text-slate-400 mt-1 leading-tight">Display Order (lower = first)</p>
              </div>
            </div>
          </div>
        </div>
